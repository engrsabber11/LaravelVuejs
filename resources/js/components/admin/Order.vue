	          ...
	          <div class="card">
	            <div class="card-header">
	              	...
	              	<div class="card-tools">
	            		<button style="color: #fff;" class="btn btn-success" @click="create">Add Order</button>
	        		</div>
	            </div>
	            <div class="card-body">
	            	<div class="mb-3">
	              ...
	              <table id="example2" class="table table-striped table-bordered table-hover">
	                ...
	                <tbody>                
		                <tr v-for="(order , index) in orders" align="center">
		                  <td>{{ index+1 }}</td>
		                  <td>{{ order.name}}</td>
		                  <td>{{ order.email}}</td>
		                  <td>{{ order.phone}}</td>
	      ...
	      <form @submit.prevent="add_category()" @keydown="form.onKeydown($event)">
      		<div class="modal-body">
	        	<div class="form-group">
	        		...
	        		<input type="text" v-model="form.name" id="title" class="form-control" name="title" placeholder="Enter Post Title" :class="{ 'is-invalid': form.errors.has('title') }" required="">
	        		<has-error :form="form" field="title"></has-error>
	        	</div>
	        	<div class="form-group">
	        		<label for="name">Post Description</label>
	        		<textarea v-model="form.email" id="description" class="form-control" name="description" placeholder="Enter Post Description" :class="{ 'is-invalid': form.errors.has('description') }" required=""></textarea>
